...
function response_(id,ok,data,error){const payload=JSON.stringify({source:'honyong-admin',requestId:id,ok,data,error}).replace(/</g,'\\u003c');return HtmlService.createHtmlOutput('<!doctype html><meta charset="utf-8"><script>window.parent.postMessage('+payload+',"*");<\\/script>').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)}
