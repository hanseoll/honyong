...
function renderQuickStatus(){
  ...
  $('quickDebtCount').textContent=totalDebts;
  $('quickFanCount').textContent=grouped.filter(f=>f.total>0).length;
  $('quickHonyoCount').textContent=data.honyodan.length;
  $('quickLatestBroadcast').textContent=data.reports?.[0]?.date||'-';
}
