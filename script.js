...
function gvizUrl(spreadsheetId, sheetName, callbackName) {
  return `https://docs.google.com/spreadsheets/d/${encodeURIComponent(spreadsheetId)}/gviz/tq?tqx=responseHandler:${encodeURIComponent(callbackName)}&sheet=${encodeURIComponent(sheetName)}`;
}
...
function gvizTableToObjects(response) {
  ...
  const headers = table.cols.map((col, index) =>
    String(col.label || col.id || `col${index + 1}`).trim()
  );
  ...
  return table.rows
    .map(row => {
      ...
      return Object.fromEntries(headers.map((header, index) => {
        ...
        const value = cell == null ? '' : (cell.f ?? cell.v ?? '');
...
function fetchSheet(spreadsheetId, sheetName) {
  return new Promise((resolve, reject) => {
    const callbackName = `__honyongGviz_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const script = document.createElement('script');
    const timeout = setTimeout(() => {
      cleanup();
      reject(new Error(`"${sheetName}" 시트 응답 시간이 초과됐어요.`));
    }, 15000);
    ...
    window[callbackName] = response => {
      try {
        resolve(gvizTableToObjects(response));
      } catch (error) {
        reject(new Error(`"${sheetName}" 시트 오류: ${error.message}`));
      } finally {
        cleanup();
      }
    };
    ...
    script.onerror = () => {
      cleanup();
      reject(new Error(`"${sheetName}" 시트를 불러오지 못했어요. 웹 게시와 공유 권한을 확인하세요.`));
    };
